...
def render_one_iter(renderer, scene, sensor, height, width, cluster,
                    ...
                    fg_depth=2, add_bridge_segments=True):
    ...
    return renderer._render_iterate_vec(
        ...
        final_gather_depth=fg_depth, add_bridge_segments=add_bridge_segments,
    )
...
def main():
    ...
    ap.add_argument("--no-bridge", action="store_true",
                    help="disable the to-light BRIDGE technique (next-event "
                         "estimation as its own path-construction technique). "
                         "ON by default, matching render_vec; with it OFF, "
                         "direct light reaches the cache only via BSDF-sampled "
                         "emitter hits (the high-variance arm the bridge "
                         "technique exists to fix). Use to A/B the bridge.")
    ...
    bridge_tag = "bridge" if bridge_on else "nobridge"
    out = os.path.join(os.path.dirname(__file__),
                       f"G_convergence_{suffix}_{bridge_tag}_i{args.max_iter}_c{args.c}_N{args.N}_alpha{args.alpha}_fg{args.fg_depth}_iter.png")
