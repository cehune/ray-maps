...
bool AABB::intersect(const Ray& ray, float& t_enter, float& t_exit) const {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // if direction is ~0, ray is parallel to slab
        if (std::abs(axesDirection) < 1e-8f) { // if exactly on boundary we might get
            if (axesOrigin < mn || axesOrigin > mx) return false; // outside bounds
            continue;
        }
